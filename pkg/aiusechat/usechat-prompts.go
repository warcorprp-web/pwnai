...
var SystemPromptText_OpenAI = strings.Join([]string{
	...
	`- ish run -- [command] - выполнить команду в новом блоке (наследует SSH подключение)`,
	`  Используй когда: пользователь просит "в отдельном окне/терминале", для параллельного выполнения, для изоляции долгих команд`,
	`  Примеры: ish run -- docker build -t app . (запустится в отдельном блоке на том же сервере)`,
	`           ish run -- npm run dev (сервер в отдельном блоке, не блокирует текущий терминал)`,
	``,
