...
func getSystemPrompt(apiType string, model string, isBuilder bool, hasToolsCapability bool, widgetAccess bool, aiMode string) []string {
	...
	// PwnAI режим - используем специальный промпт
	if aiMode == "pwnai@default" {
		return []string{pentest.PwnAISystemPrompt}
	}
...
func sendAIMetricsTelemetry(ctx context.Context, metrics *uctypes.AIMetrics) {
	event := telemetrydata.MakeTEvent("waveai:post", telemetrydata.TEventProps{
		telemetrydata.WaveAIAPIType:              metrics.Usage.APIType,
		telemetrydata.WaveAIModel:                metrics.Usage.Model,
		telemetrydata.WaveAIChatId:               metrics.ChatId,
		telemetrydata.WaveAIStepNum:              metrics.StepNum,
		telemetrydata.WaveAIInputTokens:          metrics.Usage.InputTokens,
		telemetrydata.WaveAIOutputTokens:         metrics.Usage.OutputTokens,
		telemetrydata.WaveAINativeWebSearchCount: metrics.Usage.NativeWebSearchCount,
		telemetrydata.WaveAIRequestCount:         metrics.RequestCount,
		telemetrydata.WaveAIToolUseCount:         metrics.ToolUseCount,
		telemetrydata.WaveAIToolUseErrorCount:    metrics.ToolUseErrorCount,
		telemetrydata.WaveAIToolDetail:           metrics.ToolDetail,
		telemetrydata.WaveAIPremiumReq:           metrics.PremiumReqCount,
		telemetrydata.WaveAIProxyReq:             metrics.ProxyReqCount,
		telemetrydata.WaveAIHadError:             metrics.HadError,
		telemetrydata.WaveAIImageCount:           metrics.ImageCount,
		telemetrydata.WaveAIPDFCount:             metrics.PDFCount,
